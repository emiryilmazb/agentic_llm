...
def load_dynamic_tools(server: MCPServer) -> None:
    """
    Load and register all dynamic tools from the app/tools/dynamic directory.
    
    Args:
        server: The MCPServer instance to register tools with
    """
    try:
        # Try to import tools from app.tools.dynamic
        current_dir = Path(__file__).parent.parent
        package_path = current_dir / "tools" / "dynamic"
        ...
        for _, module_name, is_pkg in pkgutil.iter_modules([str(package_path)]):
            ...
            try:
                # Import the module
                module = importlib.import_module(f"app.tools.dynamic.{module_name}")
