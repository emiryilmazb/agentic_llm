...
class AgenticCharacter:
    ...
    def get_response(self, user_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if we received a direct response instead of a tool
            if not created_tool and tool_info and "direct_response" in tool_info:
                direct_response = tool_info["direct_response"]
...
{self.name}:"""
    ...
    def _check_and_create_tool(self, user_message: str) -> Tuple[bool, Optional[str], Optional[Dict[str, Any]]]:
        ...
        """
        try:
            # Use the DynamicToolManager to check if a tool needs to be created
            # The manager will handle checking existing tools and determining if a new one is needed
    ...
    def save(self, db: Session) -> None:
        ...
        CharacterService.save_character_data(db, self.name, character_data)
    
    @classmethod
    def load(cls, db: Session, character_name: str) -> Optional['AgenticCharacter']:
        """
